                    ...
                    if (Bpsection) {
						...
						async function fetchVideoInChunks(video, videoURL) {
							...
							async function appendNextChunk() {
								...
								// Append this chunk to the array of video data
								videoData.push(videoChunk);

								const videoBlob = new Blob(videoData, { type: 'video/mp4' });
								const nextBlobURL = URL.createObjectURL(videoBlob);

								// Set the video source to the new Blob URL, maintaining continuous playback
							...
							// As the video plays, monitor the buffer and append more chunks
							video.addEventListener('timeupdate', () => {
								const bufferedEnd = video.buffered.length > 0 ? video.buffered.end(0) : 0;
								const currentTime = video.currentTime;

								// Fetch the next chunk when the video is close to the end of the buffered content
